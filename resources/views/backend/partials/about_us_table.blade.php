	...
	<table class="table table-borderless table-theme" style="width:100%;">
		<thead>
			<tr>
				...
				<th class="text-left" style="width:35%">{{ __('Title') }} </th>
				<th class="text-left" style="width:10%">{{ __('Manage Page') }} </th>
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-left">{{ $row->page_type }}</td> 
				<td class="text-left">{{ $row->url }}</td> 
				@if ($row->image != '')
				<td class="text-center"><div class="table_col_image"><img src="{{ url('/') }}/media/{{ $row->image }}" /></div></td>
				@else
				<td class="text-center"><div class="table_col_image"><img src="{{ url('/') }}/backend/images/album_icon.png" /></div></td>
				@endif
