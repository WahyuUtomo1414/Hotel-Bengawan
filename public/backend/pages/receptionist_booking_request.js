...
function onPaginationDataLoad(page) {
	...
	$.ajax({
		url:base_url + "/receptionist/getBookingRequestTableData?page="+page
		+"&search="+$("#search").val(),
...
function onRefreshData() {
	...
	$.ajax({
		url:base_url + "/receptionist/getBookingRequestTableData?search="+$("#search").val(),
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onFilterAction() {
	...
	$.ajax({
		url: base_url + "/receptionist/getBookingRequestTableData?start_date="+start_date+"&end_date="+end_date,
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onExcelExport() {
	...
	var FinalPath = base_url + "/receptionist/excel-export?booking_status_id=1&start_date="+start_date+"&end_date="+end_date;
...
function onCSVExport() {
	...
	var FinalPath = base_url + "/receptionist/csv-export?booking_status_id=1&start_date="+start_date+"&end_date="+end_date;
